...
public class H2Queries {
	...
	public static String toDbKey(AnnotationAction action) {
		switch (action) {
		case MIXED:
			return "M";
		case PICKED:
	...
	public static final String SELECT_FEATURE_BY_ID = "SELECT FEATURE.HASHCODE, FEATURE.NAME, FEATURE.VALUE, FEATURE.LEVEL, FEATURE.TOKENIZABLE, FEATURE.ID FROM FEATURE WHERE FEATURE.ID = ? LIMIT 1";
	...
	public static final String SELECT_BUNDLE_ID = "SELECT ID FROM BUNDLE WHERE FILE = ?";
	//
	...
	public static final String CREATE_TABLE_ANNOTATION_LOG = "CREATE TABLE IF NOT EXISTS ANNOTATION_LOG (ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY, PORTPAIR INT NOT NULL, BUNDLE INT NOT NULL, RECOMMENDED VARCHAR NOT NULL, ACTION VARCHAR(1) NOT NULL, ANNOTATIONS INT NOT NULL, DURATION INT NOT NULL, FROMREC INT NOT NULL, AVGRANK DOUBLE NOT NULL, AVGREL DOUBLE NOT NULL, FOREIGN KEY (PORTPAIR) REFERENCES PORTPAIR(ID))";
	public static final String ALTER_TABLE_ANNOTATION_LOG_ADD_COLUMN_AVGSUPPORT = "ALTER TABLE ANNOTATION_LOG ADD COLUMN IF NOT EXISTS AVGSUPPORT DOUBLE DEFAULT 0 NOT NULL;";
	public static final String ALTER_TABLE_ANNOTATION_LOG_ADD_COLUMN_AVGCONFIDENCE = "ALTER TABLE ANNOTATION_LOG ADD COLUMN IF NOT EXISTS AVGCONFIDENCE DOUBLE DEFAULT 0 NOT NULL;";
	public static final String INSERT_ANNOTATION_LOG = "INSERT INTO ANNOTATION_LOG (PORTPAIR, BUNDLE, RECOMMENDED, ACTION, ANNOTATIONS, DURATION, FROMREC, AVGRANK, AVGREL, AVGCONFIDENCE, AVGSUPPORT) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
	public static final String INSERT_ANNOTATION_UNIT = "INSERT INTO ANNOTATION_UNIT (PORTPAIR, ANNOTATION) VALUES (?,?)";
	public static final String SELECT_ALL_ANNOTATIONS = "SELECT BUNDLE.FILE, PORTPAIR.NAME, ANNOTATION FROM BUNDLE, ANNOTATION_UNIT, PORTPAIR WHERE BUNDLE.ID = PORTPAIR.BUNDLE AND PORTPAIR.ID = ANNOTATION_UNIT.PORTPAIR";
	public static final String SELECT_ANNOTATION_LOGS = "SELECT BUNDLE.FILE, PORTPAIR.NAME, U.ANNOTATION, L.RECOMMENDED, L.ACTION, L.ID, L.ANNOTATIONS, L.DURATION, L.FROMREC, L.AVGRANK, L.AVGREL, L.AVGCONFIDENCE, L.AVGSUPPORT FROM BUNDLE, PORTPAIR, ANNOTATION_LOG L LEFT OUTER JOIN ANNOTATION_UNIT U ON U.PORTPAIR = L.PORTPAIR WHERE BUNDLE.ID = PORTPAIR.BUNDLE AND PORTPAIR.ID = L.PORTPAIR ORDER BY L.ID";
	public static final String SELECT_ANNOTATIONS_OF_BUNDLE = "SELECT PORTPAIR.NAME, ANNOTATION_UNIT.ANNOTATION FROM BUNDLE, PORTPAIR LEFT OUTER JOIN ANNOTATION_UNIT ON PORTPAIR.ID = ANNOTATION_UNIT.PORTPAIR WHERE BUNDLE.FILE = ? AND PORTPAIR.BUNDLE = BUNDLE.ID";
	public static final String SELECT_ANNOTATION_OF_PORTPAIR = "SELECT ANNOTATION_UNIT.ANNOTATION FROM ANNOTATION_UNIT, PORTPAIR WHERE PORTPAIR.NAME = ? AND PORTPAIR.ID = ANNOTATION_UNIT.PORTPAIR LIMIT 1";
	public static final String SELECT_BUNDLE_NEVER_ANNOTATED = "SELECT FILE FROM BUNDLE WHERE ID NOT IN (SELECT BUNDLE FROM PORTPAIR WHERE PORTPAIR.ID IN (SELECT PORTPAIR FROM ANNOTATION_UNIT))";
	public static final String SELECT_BUNDLE_ANNOTATING = "SELECT ANNOTATION_LOG.BUNDLE, BUNDLE.FILE FROM ANNOTATION_LOG LEFT JOIN BUNDLE ON BUNDLE.ID=BUNDLE, PORTPAIR WHERE ACTION != 'S' AND PORTPAIR!=PORTPAIR.ID LIMIT 1";
	public static final String SELECT_BUNDLE_PORTPAIR_ANNOTATION_STATUS = "SELECT (SELECT FILE FROM BUNDLE WHERE ID=BUNDLE), COUNT(ID), (ANNOTATION_UNIT.PORTPAIR IS NOT NULL) AS STATUS FROM PORTPAIR LEFT OUTER JOIN ANNOTATION_UNIT ON ID=ANNOTATION_UNIT.PORTPAIR GROUP BY BUNDLE, STATUS ORDER BY BUNDLE, STATUS DESC";
	public static final String SELECT_BUNDLE_PORTPAIR_ANNOTATION_PROGRESS = "SELECT (SELECT FILE FROM BUNDLE WHERE ID=BUNDLE) AS BUNDLE, COUNT(ANNOTATION_UNIT.PORTPAIR) AS ANNOTATED, (SELECT COUNT(B.ID) FROM PORTPAIR AS B WHERE B.BUNDLE=A.BUNDLE) AS TOTAL FROM PORTPAIR A LEFT OUTER JOIN ANNOTATION_UNIT ON ID=ANNOTATION_UNIT.PORTPAIR GROUP BY BUNDLE ORDER BY BUNDLE";
	public static final String SELECT_PORTPAIR_IS_ANNOTATED = "SELECT TRUE FROM ANNOTATION_UNIT, PORTPAIR WHERE PORTPAIR.ID = ANNOTATION_UNIT.PORTPAIR AND PORTPAIR.NAME = ?";
}
